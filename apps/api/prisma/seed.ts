...
async function main(): Promise<void> {
  ...
  const operatorEmail = process.env.SEED_ADMIN_EMAIL ?? 'admin@veyra-pharma.example';
  const oneTimePassword = process.env.SEED_ADMIN_OTP ?? randomBytes(9).toString('base64url');

  const company = await prisma.company.upsert({
    where: { id: 'seed-company-veyra-pharma' },
    update: {},
    create: {
      id: 'seed-company-veyra-pharma',
      name: 'Veyra Pharma',
      legalName: 'Veyra Pharma Ltd.',
      primaryDomain: 'veyra-pharma.example',
      domains: { create: [{ domain: 'veyra-pharma.example', verified: true }] },
      tenant: { create: { plan: 'pilot', status: 'active' } },
    },
    include: { tenant: true },
  });
  ...
  await prisma.tenantMember.upsert({
    where: { tenantId_userId: { tenantId: company.tenant!.id, userId: user.id } },
    update: {},
    create: { tenantId: company.tenant!.id, userId: user.id, role: 'owner' },
  });
